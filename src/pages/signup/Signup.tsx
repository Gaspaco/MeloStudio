// Signup and the form checks live here. The route handles the page change after.
...
const Signup: Component<{ onBack?: () => void; onLogin: (email?: string) => void; onSuccess?: () => void }> = (props) => {
  ...
  const handleSocialLogin = async (provider: "google" | "twitter") => {
    try {
      if (provider === "twitter") {
        ...
        await socialAuthClient.signIn.social({ provider: "twitter", callbackURL: "/dashboard" });
        return;
      }
      markAuthProvider("neon");
      await neonAuthClient.signIn.social({ provider, callbackURL: "/dashboard" });
    } catch (err: any) {
  ...
  onMount(() => {
    ...
    m.fromTo(".signup__scroll-arrow", { opacity: 0, scale: 0.8 }, { opacity: 1, scale: 1, duration: 0.8, ease: "back.out(1.5)" }, 0.6);
  ...
  return (
    <div ref={(el) => { pageRef = el; }} class="signup">
      ...
      <div ref={(el) => { heroRef = el; }} class="signup__hero-text">
        ...
        <button
          class="signup__scroll-arrow"
          onClick={() => pageRef?.scrollTo({ top: pageRef.scrollHeight, behavior: 'smooth' })}
          title="Scroll to form"
        >
          <span class="signup__mouse">
            <span class="signup__mouse-wheel" />
          </span>
        </button>
      </div>
      ...
      <div ref={(el) => { formRef = el; }} class="signup__form-section">
        ...
        <form class="signup__form" onSubmit={handleSubmit}>
          ...
          <div class="signup__divider">
            <div class="signup__divider-line" />
            <span>or</span>
            <div class="signup__divider-line" />
          </div>

          <div class="signup__socials">
            <button type="button" class="signup__social" onClick={() => handleSocialLogin("google")}>
            ...
            <button type="button" class="signup__social" onClick={() => handleSocialLogin("twitter")}>
