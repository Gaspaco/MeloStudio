...
const ActivityChart: Component<{ projects: Accessor<ProfileProject[]> }> = (p) => {
  ...
  const onMouseMove = (e: MouseEvent & { currentTarget: HTMLDivElement }) => {
    ...
    for (let i = 0; i < pointPixels.length; i++) {
      const pointX = pointPixels[i];
      if (pointX === undefined) continue;
      const d = Math.abs(pointX - mx);
      if (d < minDist) { minDist = d; closest = i; }
    }
    const sortedProjects = p.projects().slice().sort((a, b) => new Date(a.updatedAtRaw).getTime() - new Date(b.updatedAtRaw).getTime());
    const proj = sortedProjects[closest];
    const pointX = pointPixels[closest];
    if (!proj || pointX === undefined) return;
    setHovered({ name: proj.name, bpm: proj.bpm, x: pointX });
  };
                    ...
                    <div class="db__pp-act-block">
                      ...
                      <div class="db__pp-act-block-inner">
                        ...
                        <div class="db__pp-act-block-wave">
                          <Show when={clipUrls()[p.id]}>
                            {(clipUrl) => <WaveformCanvas url={clipUrl()} color={p.color} />}
                          </Show>
                        </div>
